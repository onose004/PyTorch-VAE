...
class ADEXperiment(pl.LightningModule):

    def __init__(self,
                 vae_model: BaseVAE,
                 params: dict
                 ) -> None:
        super(ADEXperiment, self).__init__()
    ...
    def train_dataloader(self):
        ...
        if self.params['dataset'] == 'MNISTB':
            vol_np = np.load(os.path.join(self.params['train_data_path']))
            labels_np = np.load(os.path.join(self.params['train_labels_path']))
            data = torch.from_numpy(vol_np).float()
            targets = torch.from_numpy(labels_np).int()
    ...
    def val_dataloader(self):
        ...
        if self.params['dataset'] == 'MNISTB':
            vol_np = np.load(os.path.join(self.params['test_data_path']))
            labels_np = np.load(os.path.join(self.params['test_labels_path']))
            data = torch.from_numpy(vol_np).float()
            targets = torch.from_numpy(labels_np).int()
            dataset = torch.utils.data.TensorDataset(data, targets)
